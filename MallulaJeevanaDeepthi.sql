...
/*Write a sql query to find the total number
 of users who were born in the year 1999?*/
SELECT COUNT(USERID) FROM FACEBOOK WHERE DOB_YEAR=1999;


/*Write a SQL query to count the total number 
of social media users for each age group?*/
...
/*Write a sql query to find the percentage of female 
users who are using social media?*/
SELECT (COUNT(CASE WHEN GENDER = 'female' THEN 1 END) * 100.0 / COUNT(USERID)) AS PERCENTAGE FROM FACEBOOK;


/*Write a sql query to find the percentage of female 
users above the age 17?*/
